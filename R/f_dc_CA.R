change_reponse  <- function(f, response, data = NULL){ # used in dc_CA and dc_CA
  # response : character

  ft <- as.character(f)
  if (any(ft %in% ".")){
    # change the dot using  data
    if (is.null(data)) stop("data should be present if formula contains a period(.).")
    if (ft[2]==".") ft <- c("~",paste(names(data), collapse = "+")) else
  ...
  if (length(ft)>3) warning("as.character(formula) has more than three elements, only its last element is included")
