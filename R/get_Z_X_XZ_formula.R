...
#'
#' @description
#' \code{get_Z_X_XZ_formula} derives the focal constraining formula and (if present) the
#' conditioning formula from a vegan formula.
...
#' @noRd
get_Z_X_XZ_formula <- function(formula, data=NULL, factors_only = FALSE){
  ...
  if (is.null(tl)){
    ...
    stop("Specify wrda or cca via a formula, so that predictor and covariate formulas can be found.")
  }
  ...
  if (!is.na(idC)){
    ...
    fC_formula <- stats::as.formula(paste("~",fC_wo_tilde, sep = ""))
  } else { fC_formula <- ~1}
#} else {fC_wo_tilde <- "NULL"; fC_formula <- NULL}
  ...
  if (!is.null(data)) {
    ...
    if (length(focal_factor)==0)focal_factor <-NULL
    if (length(Condi_factor)==0)Condi_factor <-NULL

  } else {focal_factor <- Condi_factor <- NA}

  return(list(formula_XZ=  fFocalXZ, formula_X0=  fFocalX0, formula_Z = fC_formula,
              focal_nams = focal_nams, Condi_nams= Condi_nams, formula_X1 =fFocalX1,
              focal_factor = focal_factor , Condi_factor = Condi_factor,
