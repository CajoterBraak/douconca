...
f2_orth <- function(CWM,
  ...
  if (all(intersect(rownames(CWM2CWM_ortho), names(CWM)) 
          == rownames(CWM2CWM_ortho)) ){
   CWM <- as.matrix(CWM[, rownames(CWM2CWM_ortho)])
  } else {
    if (name == "CWM") fname <- "formulaTraits" else fname <- "formulaEnv"
...
checkCWM2dc_CA <- function(object, 
  ...
  CWM2ortho <- with(object, f2_orth(CWM,formulaTraits, data$dataTraits, 
                                    weights$columns, weights$rows, name = "CWM"))
  ...
  if (!is.null(object$SNC) && !is.null(object$weights$rows)) {
    SNC2ortho <- with(object, f2_orth(SNC,formulaEnv,data$dataEnv, 
                                      weights$rows,weights$columns, name = "SNC"))
